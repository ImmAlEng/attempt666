...
typedef struct s_cmd
{
	...
	t_dlist			*tokens;
	pid_t			pid;
	int				fd_in;//nicht verwendet
	int				fd_out;//nicht verwendet
	int				pipe_in[2];//nicht verwendet
	int				pipe_out[2];//nicht verwendet
	int				pipe[2];//regular pipe
	int				her_pipe[2];//for heredoc
	int				n_heredoc;//number of heredocs in each given command
	bool			has_heredoc;
	bool			is_builtin;
	bool			needs_fork;
	int				exit_status;
...
{//noch nix von hier benutzt ausser cmds, n_cmds und env;
	char			*line;
	t_dlist			*pipeline;
	t_cmd			**cmds;
	size_t			n_cmds;
...
char	*ft_get_path_exec(char **dirs, char *cmd, int i);//added strcpy and strcat into libft
...
void	ft_redir_her(t_data *data, int c_i, char *del, bool quoted);//added strcmp into libft
